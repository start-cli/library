...
When adding new content (instructions, identity bullets, restrictions), follow the authoring rules from `tasks:start/module/role/create` — in particular the required Instructions directives (style, quality, comment discipline) and the Restrictions guidance (intrinsic role constraints, not task directives). To view the create task content, run:

```bash
start get tasks:start/module/role/create
```
...
Before publishing, confirm the role is still complete after the change: every affected mode is authored consistently, and any change to the shared Skill Set or base Restrictions is reflected across all three modes.
...
It is the single source for validation, version determination, the mandatory index update, the single module-plus-index commit, the tag pushes, the registry publish, verification, and closing any related GitHub issue. Publish only the affected modes, applying the per-module steps and one index entry bump per affected mode, as the workflow describes.
